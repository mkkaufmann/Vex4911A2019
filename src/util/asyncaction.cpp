...
object->targetAmount << ", " << (object->count >= object->targetAmount ) <<
std::endl;
...
public:
        ...
        Action(std::function<void()> _actionMethod, std::function<bool()>
finished){ actionMethod = _actionMethod; finishedMethod = finished; firstRun =
true;
        }
...
bool AsyncAction::isFinished() { return action.isFinished(); }
// private:
...
// 	AsyncAction action2 =
// *AsyncActionFactory::makeAsyncAction()->hasTrigger(ActionTrigger(test1.reachedAmount))->hasAction(Action(test2.incrementCount,
// test2.reachedAmount)); 	AsyncAction action1 =
// *AsyncActionFactory::makeAsyncAction()->hasTrigger(ActionTrigger([]()->bool{return
// true;}))->hasAction(Action(test1.incrementCount, test1.reachedAmount));
// 	AsyncAction action3 =
// *AsyncActionFactory::makeAsyncAction()->hasTrigger(ActionTrigger(test1.reachedAmount))->hasAction(Action([&](){std::cout<<test1.count<<std::endl;},
// [](){return true;})); 	AsyncAction action4 =
