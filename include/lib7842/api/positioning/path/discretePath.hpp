 ...
 */
template <typename T>
class DiscretePath : public TemplatePath<DiscretePath<T>> {
public:
   ...
   * @param ipath The array of points
   */
  DiscretePath(const std::initializer_list<T> &ipath)
      : DiscretePath(std::vector<T>(ipath)){};
  ...
  /**
   * Smoothen the path. This is a very expensive operation that smooths the
   * points in the path. The first parameter is how important each point is - a
   * lower value means more smoothing. The second parameter is how much variance
   * between the distances of the points is allowed. A smaller value will
   * stretch the points out and make a more even path. Instead of using this
   * function, use this path as the input to QuinticSpline, which does a better
   * job of smoothing.
   *
   ...
   * @param  iend        Whether to return the end of the segment. This can be
   * turned off to prevent the start of the next segment from being redundant.
  ...
  SimplePath generate(const QLength &iresolution, bool iend = true) const {
    ...
    // if the path is more than 1 point and the end point is required - return
    // last point if there is only one point, always return it
