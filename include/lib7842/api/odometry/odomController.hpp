...
public:
   ...
   */
  virtual void
  moveDistanceAtAngle(const QLength &distance,
  ...
  /**
   * A Settler that is used for turning which uses the turning pid's isSettled()
   * method
   */
  static bool defaultTurnSettler(const OdomController &odom);

  /**
   * A Settler that is used for driving which uses the distance pid's
   * isSettled() method
   */
  static bool defaultDriveSettler(const OdomController &odom);

  /**
   * A Settler that is used for driving which uses the distance and angle pid's
   * isSettled() method
   */
  ...
  /**
   * Make a Settler that exits when both angle and distance error is within
   * given range.
   * @param angle The angle error threshold
   * @param distance The distance error threshold
   */
