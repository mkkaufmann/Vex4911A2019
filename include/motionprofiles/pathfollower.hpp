...
public:
  ...
  void run() {
    ...
    if (Util::epsilonEquals(lastRecordedPosition.getX(), target.getX(),
                            xPID.getEpsilon()) &&
        Util::epsilonEquals(lastRecordedPosition.getY(), target.getY(),
                            yPID.getEpsilon()) &&
        Util::epsilonEquals(lastRecordedOrientation, fmod(rotTarget, M_2_PI),
                            tPID.getEpsilon())) {
      settledIterations++;
    }
    ...
    xSpeed =
        std::max(std::min((int)(xPID.getOutput(target.getX() -
                                               lastRecordedPosition.getX())),
                          127),
                 -127);
    ySpeed =
        std::max(std::min((int)(xPID.getOutput(target.getY() -
                                               lastRecordedPosition.getY())),
                          127),
                 -127);
    tSpeed = std::max(std::min((int)(xPID.getOutput(fmod(rotTarget, M_2_PI) -
                                                    lastRecordedOrientation)),
                               127),
                      -127);
    // update drivetrain
