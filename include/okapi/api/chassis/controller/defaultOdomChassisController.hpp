...
public:
  /**
   * Odometry based chassis controller that moves using the V5 motor's
   * integrated control. Spins up a task at the default priority plus 1 for
  ...
  DefaultOdomChassisController(
      const TimeUtil &itimeUtil, std::unique_ptr<Odometry> iodometry,
